...
%
% Requires: The mapping tolbox and the statistics toolbox
...
if try_ver==1
    try % using hardcoded version (faster)
        ixVersion=strfind(onlinedata,'version-number="');
        onlinedata=onlinedata(ixVersion(1)+16:ixVersion(1)+50);
        ixsspan=strfind(onlinedata,'"');
        ...
        if version_online>version
            warndlg(['NOTE: A more recent version (ver. ',onlinedata,') of VoronoiLimit is now available on the mathworks fileexchange. Currently running version ',num2str(version)])
            pause(2)
        end
    catch me
        if user==1
    ...
    for ij=1:length(C)
        if sum(allVixinp(C{ij}))~=length(C{ij})
            ...
            if poly_ok(ij)==1
                ...
                hr=V(C{ij},1);
                vr=V(C{ij},2);
                [hrCW, vrCW, idx] = orderClockwise(hr, vr);
                hr=hrCW;
                vr=vrCW;
                [xb, yb] = polybool('intersection',crs(:,1),crs(:,2),hr,vr);
...
%                                     nb=nb(poly_ok(nb)==0);
                                    if ~isempty(nb)
                                        ...
                                        for in=1:length(nb)
                                            ...
                                            hr2=poly(:,1);
                                            vr2=poly(:,2);
                                            [hrCW2, vrCW2, idx] = orderClockwise(hr2, vr2);
                                            hr2=hrCW2;
                                            vr2=vrCW2;
                                            [xb,~] = polybool('intersection',hr,vr,hr2,vr2);
                                            if ~isempty(xb)
                                                overlap(in)=1;
                                            end
                                        end
