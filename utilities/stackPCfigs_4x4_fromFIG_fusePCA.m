function figOut = stackPCfigs_4x4_fromFIG_fusePCA(figfile_top, figfile_bottom, out_name)
...
topY   = botY + axH + gapY; % y of top panel -> top edge ~0.78 (room for legend)
    ...
    strTop = get(axTop,'XLabel').String;     % e.g. 'PC1 (89.3%)'
    strBot = get(axBot,'XLabel').String;     % e.g. 'PC1 (99.5%)'
    
    pctTop = regexp(strTop, '\((.*?)\)', 'tokens', 'once');
    pctBot = regexp(strBot, '\((.*?)\)', 'tokens', 'once');
    
    newStr = sprintf('PC1 (%s Top, %s Bottom)', pctTop{1}, pctBot{1});
    
    set(get(axBot,'XLabel'), 'String', newStr, 'FontSize', 6);
    set(get(axBot,'YLabel'),'FontSize',6);
